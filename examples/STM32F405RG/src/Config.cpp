...
{
    USART_DEBUG("Reading configuration from file: " << fileName << UsartLogger::ENDL);
    
    FRESULT res = readFile(fileName);
    if (res != FR_OK)
    {
        USART_DEBUG("Can not read configuration: " << res << UsartLogger::ENDL);
        return false;
    }
    else
    {
...
FRESULT Config::readFile (const char * fileName)
{
    ...
    while (f_gets(buff, MAX_LINE_LENGTH, &cfgFile) != 0)
    {
        ...
        {
            USART_DEBUG("Parameter " << name << " is not known" << UsartLogger::ENDL);
            continue;
        }
...
void Config::dump () const
{
    for (size_t i = 0; i < CfgParameter::size; ++i)
    {
        USART_DEBUG(UsartLogger::TAB << CfgParameter::strings[i] << ": " << parameters[i] << UsartLogger::ENDL);
    }
}
